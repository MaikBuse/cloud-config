module "kube-hetzner" {
  ...
  # * Your ssh public key
  ssh_public_key = file("${path.module}/keys/maikbuse.pub")
  ...
  hcloud_ssh_key_id = "contact@maikbuse.com"
  ...
  control_plane_nodepools = [
    {
      ...
      kubelet_args = ["kube-reserved=cpu=250m,memory=1000Mi,ephemeral-storage=1Gi", "system-reserved=cpu=250m,memory=250Mi"]
  ...
  firewall_kube_api_source = var.source_addresses
  ...
  firewall_ssh_source = var.source_addresses

  # Adding extra firewall rules, like opening a port
  # More info on the format here https://registry.terraform.io/providers/hetznercloud/hcloud/latest/docs/resources/firewall
  extra_firewall_rules = [
    {
      description : "Direct WireGuard tunnels use UDP with source port 41641. Recommended allowing to any destination to ensure connectivity across various networks.",
      direction : "out",
      protocol : "udp",
      port : "41641", // Note: If your system differentiates source/destination ports, adjust accordingly.
      source_ips : [],
      destination_ips : ["0.0.0.0/0", "::/0"]
    },
    {
      description : "The STUN protocol on port 3478 lets a machine behind NAT figure out its public IP and NAT type. Used for Tailscale's NAT traversal.",
      direction : "out",
      protocol : "udp",
      port : "3478",
      source_ips : [],
  ...
  additional_tls_sans = ["hetzner-kube-api"]
  ...
  #   kubectl apply -f /var/user_kustomize/argocd-app-of-apps.yaml
  # EOT
